...
for i in range(1, N + 1):
    live[i] = live[i - 1] % 1000
    born_psum[i] += born_psum[i - 1] % 1000
    if i >= death:
        live[i] -= born[i - death]  # (i-death)일차에 태어난 개체수 차감

    # i-end일차 이후부터 i-start일차까지에 태어난 개체로부터 새 개체 생성
    if i >= end:
        born[i] = (born_psum[i - start] - born_psum[i - end]) % 1000
        live[i] += born[i] % 1000
        born_psum[i] = (born_psum[i - 1] + born[i]) % 1000
    elif i >= start:
        born[i] = born_psum[i - start] % 1000  # i-start일차 이후 태어난 개체로부터 새 개체 생성
        live[i] += born[i] % 1000
        born_psum[i] = (born_psum[i - 1] + born[i]) % 1000
